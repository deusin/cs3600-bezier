...
Slider redSlider(10, 110, screen_x - 100, 150, 1, 0, 0, 0);
Slider greenSlider(10, 60, screen_x - 100, 100, 0, 1, 0, 0);
Slider blueSlider(10, 10, screen_x - 100, 50, 0, 0, 1, 0);
...
void mouseButton(int mouse_button, int state, int x, int y)
{
    ...
    if (mouse_button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        if (selectedBezier != -1)
        {
            ...
            {
                curves[selectedBezier].red = redSlider.GetValue();
                sliderChanged = true;
                sliderClicked = true;
            }
            if (greenSlider.Click(clickPoint))
            {
                curves[selectedBezier].green = greenSlider.GetValue();
                sliderChanged = true;
                sliderClicked = true;
            }
            if (blueSlider.Click(clickPoint))
            {
                curves[selectedBezier].blue = blueSlider.GetValue();
                sliderChanged = true;
                sliderClicked = true;
            }
        }
        ...
        if (!sliderChanged)
        {
            for (size_t i = 0; i < curves.size(); i++)
            {
                ...
                if (sel != -1)
                {
                    ...
                    redSlider.SetValue(curves[i].red);
                    greenSlider.SetValue(curves[i].green);
                    blueSlider.SetValue(curves[i].blue);
                    break;
                }
                else
                {
...
void mouseMove(int x, int y)
{
    ...
    if (sliderClicked)
    {
        ...
        if (redSlider.Click(dragPoint))
        {
            curves[selectedBezier].red = redSlider.GetValue();
        }
        if (greenSlider.Click(dragPoint))
        {
            curves[selectedBezier].green = greenSlider.GetValue();
        }
        if (blueSlider.Click(dragPoint))
        {
            curves[selectedBezier].blue = blueSlider.GetValue();
        }
        glutPostRedisplay();
